...
Animate( void )
{
	...
	for (int i = 0; i < NUMPARTICLES; i++){
		...
		if (Particles[i].y >= Particles[i].endHeight){
			...
			Particles[i].vx = Ranf(vlow, vhigh);
			Particles[i].vz = Ranf(vlow, vhigh);
...
InitGraphics(void)
{
	...
	float top = 80.0;
	for (int i = 0; i < NUMPARTICLES; i++){
		Particles[i].endHeight = Ranf(bottom, top);
		
		Particles[i].x0 = Ranf(low,high);
		Particles[i].y0 = 0;
		Particles[i].z0 = Ranf(low, high);
		...
		Particles[i].vx0 = 0;
		Particles[i].vy0 = Ranf(vlow, vhigh);
		Particles[i].vz0 = 0;
		...
		Particles[i].t0 = Ranf(vlow, high);
		Particles[i].t1 = Ranf(vlow, high);
	}
